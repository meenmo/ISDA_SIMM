...
# # Concentration Threshold 별 해당 bucket 추출
# def bucket_list_CT(df, CT_list, i):
#     return [int(b) for b in list(df[CT_list[i]]) if str(b) != 'nan']
...
def rho(risk_class,index1=None,index2=None,bucket=None):
    ...
    if risk_class in gv.list_rates:
        return pd.DataFrame(
            ir_corr,
            columns=gv.tenor_list,
            index=gv.tenor_list
        )[index1][index2]

    elif risk_class in gv.list_creditQ:
